...
export const startLiveSession = asyncHandler(async (req, res) => {
  ...
  // Check if user is the mentor of this class
  if (classObj.mentor.toString() !== userId.toString()) {
    res.status(403);
    throw new Error('Only the mentor can start a live session');
  }

  // Check if class is a live class
  if (classObj.contentType !== 'live') {
    res.status(400);
    throw new Error('Only live classes can start live sessions');
  }

  // Update class status to indicate live session is active
  classObj.isLive = true;
  classObj.liveSessionStartTime = new Date();
  await classObj.save();

  res.status(200).json({
    message: 'Live session started successfully',
    classId: classObj._id,
    startTime: classObj.liveSessionStartTime
  });
});
